...
A public listing or broadcast plane may advertise a bounded target label, transport route, protocol/profile version, and short-lived invitation locator. It must not broadcast the pairing secret, proof, accepted grant, participant/private state, or native owner handle. Treat a discovered room, stream, peer UUID, or beacon as a routing hint; only the private authenticated control session can establish identity and authority.

A fixed public WebRTC namespace may also contain one-to-one SCTP/DTLS data channels. If local approval sends fresh private-session credentials over the exact requester-bound channel, call the overall service a rendezvous plane rather than claiming every public-namespace frame is discovery-only. Verify that the SDK does not broadcast or fall back to a shared WebSocket, bind the response to the request nonce and peer/channel generation, expire it, and state that signaling-provider integrity plus operator peer selection are trusted. Passive-room confidentiality is not hostile-signaling authentication. Use an out-of-band QR, independently verified key fingerprint/SAS, authenticated account service, or reviewed PAKE when the threat model must resist signaling-provider or active MITM substitution.
...
When the application already has a native reducer or domain service, treat it as the BRSP application-target authority and qualify it against the protocol's application-target invariants. Do not copy a JavaScript reference adapter into the native app or create a second reducer, state store, or authorization owner. Local Tauri actions and authenticated remote actions should converge on the same typed transition path after origin-specific authorization; BRSP, WebRTC, WebSocket, and WebView code remain bounded adapters that do not own application state.
...
Treat token entropy and token grammar as separate invariants. A canonical base64url nonce may begin with `-` or `_` even when the application's closed identifier grammar requires an alphanumeric first character. Generate owner/grant handles through one constructor that preserves the full random nonce while adding a valid fixed prefix (or use an encoding whose complete range matches the validator), and exercise many generated values against the exact renew/revoke parser. Do not let claim mint tokens that later authority operations reject probabilistically.
