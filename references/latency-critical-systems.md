...
- A fixed event sink or queue capacity does not by itself bound callback work. If the callback must scan a schedule or other retained input to discover what is due, validate a maximum callback span and event density during preparation, then enforce a constant scan/lookahead ceiling in the callback. Reject an over-dense plan before it can become active.
- Preflight the complete scientific-output and event capacity for a callback before copying samples or publishing any record. On an integrity overflow, clear the current callback output and its logical event prefix, latch a typed fault, and leave authoritative cursors unchanged; partial evidence plus audible output is not a valid failure mode.
- Treat a prepared native output plan as validated preflight data, not executable or armed state. Device-route qualification, activation policy, and the execution owner must still commit it through a closed transition before playback. Cache readiness must not imply that physical output can start.
- Distinguish an engine-owned `final frame submitted` record from device drain, DAC/display presentation, and physical onset. Completion at the software-buffer boundary is not physical timing evidence. If render state owns heap-backed media or schedules, quiesce the callback and retire/drop that state on a bounded non-real-time, non-authority owner so neither the callback nor the authority actor performs the last reference-count decrement or a potentially large deallocation.
...
Network task bounds begin before authentication. Put finite deadlines around handshake reads, writes, and close; acquire pre-upgrade admission independently for routes that must not starve each other; and use one bounded writer rather than letting many producers await the socket. Transport Ping/Pong may retire an unresponsive route, but it must not refresh an application lease or controller deadman. For reliable control, queue saturation is a fail-closed integrity fault, not permission to drop a command and forward later ones.
